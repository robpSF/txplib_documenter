...
def upload_to_contentful(txplib_file, selected_images_data,openai_description):
    ...
    for img_data in selected_images_data:
        if "image_file" in img_data:  # Check if it's an uploaded image
            # Upload the image file to Contentful
    ...
    upload_id = upload_txplib_file_to_contentful(txplib_file)
...
def upload_txplib_file_to_contentful(txplib_file):
    url = f"https://upload.contentful.com/spaces/{st.secrets['CONTENTFUL_SPACE_ID']}/uploads"
    headers = {
        "Authorization": f"Bearer {st.secrets['CONTENTFUL_ACCESS_TOKEN']}",
        "Content-Type": "application/octet-stream"
    ...
    file_binary_data = txplib_file.read()
    
    # Upload the binary file data
    response = requests.post(url, headers=headers, data=file_binary_data)
